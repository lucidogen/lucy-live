...
{ let exts = Object.keys ( HANDLERS.eval )
  ...
  { let basePath = lpath.resolve ( paths [ i ], request )
    ...
    { // try to load 'index' file
      filename = tryExtensions
      ( lpath.resolve ( basePath, 'index' )
      , exts
      )
    }
...
{ return lpath.resolve
  ( caller_id.substr ( caller_id.indexOf ( ':' )  + 1 ) )
}
...
{ let filename = resolvePath
  ( path
  , lpath.dirname ( pathFromCaller ( caller () ) )
  )
